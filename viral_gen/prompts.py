...
SYSTEM_GENERATE = textwrap.dedent(
    ...
    FORMAT:
    - "single": one tweet, <=280 chars. Set media_required=false and
      media_suggestion=null.
    - "single+media": one tweet that PAIRS with an image. Set
      media_required=true and write a FULL, production-grade image-generation
      prompt in media_suggestion (see rules below).

    MEDIA PROMPT RULES (only for "single+media") — the goal is a polished,
    professional image that VISUALLY REINFORCES the exact claim in the post.
    Write a single rich paragraph (40-80 words) that a top text-to-image model
    (Midjourney / DALL-E / Flux) could render directly. It MUST specify:
      - Subject & action: the literal thing in the image, tied to the post's
        core claim (name the same entities/numbers the post mentions)
      - Composition & framing: shot type, angle, focal point, rule-of-thirds,
        negative space for where text/overlay could sit
      - Lighting: e.g. soft key light, golden hour, studio softbox, rim light
      - Color & mood: a deliberate palette and the emotion it should evoke
      - Style & medium: e.g. editorial photography, 3D render, flat vector
        infographic, cinematic still — pick what best fits the post's tone
      - Quality cues: "sharp focus, high detail, professional, 8k, clean"
      - Aspect ratio: "16:9" for landscape or "1:1" for square
    DO NOT include any text/words/letters inside the image (image models render
    text poorly); convey the message through the visual itself. Keep it brand-safe:
    no logos you can't use, no real identifiable faces unless the post names a
    public figure, nothing that trips the seven PTOS categories.
      ...
      "candidates": [
        {
          ...
          "media_suggestion": "<full image-gen prompt per MEDIA PROMPT RULES, or null>",
